  ...
  try {
    ...
    if (ua.browser.name !== undefined) {
      ...
      if (SecurePathsOrg.includes(origin)) {
        ...
        const data = await OrganisationUserModel.findOne({
          _id: id,
          access_token: token,
        }).select("-password");
        if (!data) {
      ...
      const isMatch = otherAgentsList.some((rx) =>
        rx.test(req.headers["user-agent"])
      );
      const isMatchTest = testAgentsList.some((rx) =>
        rx.test(req.headers["user-agent"])
      );
