...
// const GoalSupportTempModel = require("../src/v1/models/goalSupportTemp.model");
...
async function getVippsAgreementStatus(req, orgId) {
  ...
  const goalSupportTempData = await GoalSupportTempModel.findOne({
    user_id: req.user.id,
  });
  if (!goalSupportTempData) {
    throw new Error("Agreement not found! Kindly try again.");
  }
  if (
    organisationData.msn === "" ||
    organisationData.msn_status !== "completed"
  ...
  const config = {
    method: "get",
    url: `${process.env.VIPPS_BASE_URL}/recurring/v3/agreements/${goalSupportTempData?.agreement_id}`,
    headers: {
      Authorization: `Bearer ${accessToken}`,
      "Ocp-Apim-Subscription-Key": process.env.VIPPS_SUBSCRIPTION_KEY,
      "Content-Type": "application/json",
...
async function getVippsAgreementStatusForUpdate(req, orgId, agreement_id) {
  const organisationData = await OrganisationModel.findById(orgId);
  if (!organisationData) {
    throw new Error("Organisation Id is invalid! Kindly try again.");
  }
...
async function createVippsMSN(orgName, orgNo, accountNo, organisationNumber) {
  ...
  const config = {
    method: "post",
    url: `${process.env.VIPPS_BASE_URL}/management/v1/product-orders`,
    data: {
      businessIdentifier: {
        scheme: "business:NO:ORG",
        id: `${organisationNumber}`,
      },
      salesUnitName: `Støtte-appen`,
      settlementBankAccount: {
        scheme: "BBAN:NO",
        id: `${accountNo}`,

      },
      pricePackageId: "4ec54be4-ebf3-4c0b-958d-51047ea8ab6a",
      ...
      complianceData: {
          subscription: {
              turnoverShare: "Ca 15%",
              periodDistribution: "Abonnementet varer til brukeren velger å si opp."
          }
      }
  },
    headers: {
...
module.exports = {
  ...
  getVippsAgreementStatusForUpdate,
